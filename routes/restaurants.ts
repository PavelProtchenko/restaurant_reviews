...
router.get("/:restaurantId/reviews", checkRestaurantExists, async (req: Request<{ restaurantId: string }>, res, next) => {
    ...
    try {
        const client = await initializeRedisClient();
        const reviewKey = reviewKeyById(restaurantId);
        const reviewIds = client.lRange(reviewKey, start, end);
        const reviews = await Promise.all((await reviewIds).map(id => client.hGetAll(reviewDetailsKeyById(id))));
...
router.delete("/:restaurantId/reviews/:reviewId", checkRestaurantExists, async (
    req: Request<{ restaurantId: string, reviewId: string }>, res, next) => {
